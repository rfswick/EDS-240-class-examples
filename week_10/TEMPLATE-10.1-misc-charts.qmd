...
#........................create bump chart.......................
salary_rank_by_year %>% 
  filter(occupation %in% top_2016) %>% 
  ggplot(aes(x = year, y =  rank, color = occupation)) +
  geom_point(shape = "|",
             size = 6) +
  geom_bump(linewidth = 1) +
  geom_text(
    data = salary_rank_by_year %>% filter(year == 2013, occupation %in% top_2016),
  ...
  geom_text(
    data = salary_rank_by_year %>%  filter(year == 2016, occupation %in% top_2016),
    aes(label = rank),
    hjust = 0,
    nudge_x = 0.1,
    family = "oxygen",
